...
*  **Produtos:** Listagem de produtos cadastrados e busca específica por nome utilizando massa de dados real. 
*  **Carrinho de Compras:** Busca de produto, adição à lista por escopo, validação de valores de integridade e limpeza de carrinho. 
...
* **Comportamento Inconsistente na Exclusão Unitária do Carrinho:** Identificou-se que, ao tentar remover os produtos individualmente pelas ações da tabela, o sistema retém o último item na tela, impossibilitando deixar o carrinho 100% vazio por exclusão unitária (falha de renderização/limpeza de estado do componente). A zeragem total só é alcançada através do botão "Limpar Lista".
    * **Ação de QA:** Foi estruturado um cenário de teste automatizado específico (**Cenário 3: Limpar Lista**) focado em acionar diretamente o botão de limpeza global, garantindo a validação de fluxo com o carrinho totalmente vazio de forma resiliente.
...
###  Instabilidade de Infraestrutura (Evidência do Bug)
